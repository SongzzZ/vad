...
class Memory(nn.Module):
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            #losses
            loss = self.gather_loss(query,keys, train)
            # read
            updated_query, softmax_score_query,softmax_score_memory = self.read(query, keys)
            #update
            updated_memory = self.update(query, keys, train)
            
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory, loss
    ...
    def gather_loss(self,query, keys, train):
        ...
        if train:
            ...
            losses = 0.1*(gathering_loss+top1_loss)
            return losses
    ...
    def read(self, query, updated_memory):
        ...
        updated_query = concat_memory.view(batch_size, h, w, 1*dims)
